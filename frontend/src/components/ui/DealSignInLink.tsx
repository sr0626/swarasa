...
export default function DealSignInLink({
  ...
  return (
    <Link
      href={href}
      aria-label={ACCESSIBLE_NAME}
      className={`inline-flex min-h-11 items-center rounded-brand-pill ${FOCUS_RING} ${className}`}
    >
      <span className="inline-flex items-center gap-1 rounded-brand-pill bg-brand-accent/10 px-2.5 py-1 text-xs font-semibold text-brand-accent underline underline-offset-2 hover:bg-brand-accent/15">
